...
class LogicCalculation {
  ...
  extraLocationCounts(generalLocation, { isDungeon, onlyProgressLocations, disableLogic }) {
    ...
    const color = LogicCalculation.#extraLocationCountsColor(numAvailable, numRemaining, anyProgress);
  ...
  static #extraLocationCountsColor(numAvailable, numRemaining, anyProgress) {
    if (numRemaining === 0) {
      return this.LOCATION_COLORS.CHECKED_LOCATION;
    }
    if (numAvailable === 0) {
